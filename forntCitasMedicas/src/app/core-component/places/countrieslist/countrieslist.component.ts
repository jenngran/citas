...
import { countries } from 'src/app/shared/model/page.model';
...
export class CountrieslistComponent {
  ...
  selectedList3: data[] = [{ value: 'Choose Region' }, { value: 'Region' }];
  initChecked = false;
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.countriesList) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getCountryList().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
