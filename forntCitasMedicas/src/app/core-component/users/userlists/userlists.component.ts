...
export class UserlistsComponent {
  ...
  initChecked = false;
  public tableData: Array<userList> = [];
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.usersUserList) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getUserList().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: userList, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<userList>(this.tableData);
